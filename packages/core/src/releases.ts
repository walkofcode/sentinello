...
// Typed as a non-empty tuple, not ReleaseEntry[]: the product has shipped releases, so getLatestRelease
...
export const RELEASE_COPY: Record<Locale, Record<string, ReleaseCopy>> = {
    ...
    fr: {
        '3.4.0': {
            ...
            items: [
                'gemnasium écrit un petit nombre d’avis avec une espace entre le comparateur et sa version — <code>&lt; 0.5.2</code> au lieu de <code>&lt;0.5.2</code> — et l’analyseur lisait cette paire comme deux jetons distincts. Le <code>&lt;</code> orphelin recevait une borne vide, et la version restée seule était mise en cache comme version exacte. L’avis <code>fresh</code> signalait donc la 0.5.2 comme vulnérable alors que 0.5.2 est justement la version qui l’a corrigé, et le signalait sans correctif disponible, puisqu’une version épinglée ne porte aucune cible de mise à jour. 19 avis sont écrits ainsi et tous étaient mal analysés : 15 perdaient entièrement leur plage, et 7 épinglaient une version que l’enregistrement lui-même donne comme correctif — <code>pg</code> en épinglait onze.',
                'Sentinello a cessé d’interpréter lui-même la syntaxe des plages npm et confie désormais chaque plage à l’implémentation de npm, ce qui élimine d’un coup toute une famille de mauvaises lectures. Pour npm, <code>&lt;=3.3</code> signifie « jusqu’à la fin de la ligne 3.3 » — lu au pied de la lettre, il s’arrêtait à 3.3.0 et manquait la 3.3.1, ce qui correspond à un avis réel de <code>converse.js</code> — et <code>=103</code> désigne toute la ligne 103 plutôt qu’un point unique, ce que <code>binaryen</code> énonce huit fois. Les plages avec accent circonflexe, tilde, <code>1.x</code> et trait d’union sont désormais lues elles aussi, alors qu’auparavant un enregistrement écrit dans l’une d’elles était écarté sans un mot. Vérifié sur les 4 696 plages de versions distinctes que gemnasium publie pour npm : les interpréter nous-mêmes divergeait de npm sur 9, déléguer ne diverge sur aucune. Seul npm est concerné — en Python, <code>==1.0</code> est une version exacte et non un joker, si bien qu’y appliquer la règle de npm inventerait des résultats.',
                'Trois choses que la lecture de npm réussit pour installer un paquet et rate pour un avis sont retenues. Une plage ne nommant aucune version — <code>*</code>, <code>x</code>, un champ vide — signifie « n’importe quelle version » quand npm installe quelque chose et « personne n’a rempli ceci » dans un avis : elle est donc refusée plutôt que transformée en un résultat contre toutes les versions jamais publiées. Un <code>||</code> traînant ne peut plus élargir à tout la plage qui le précède. Et <code>&gt;0</code> continue de signifier toutes les versions, car <code>pandora-doomsday</code> déclare ainsi son paquet malveillant et la lecture de npm laverait tout le 0.x. Par ailleurs, un avis dont le correctif annoncé se situe au niveau du début de sa propre plage affectée ou en dessous ne se voit plus substituer cette borne — cela produisait un intervalle ne correspondant à rien, autrement dit un résultat qui cesse silencieusement de signaler — et la règle écartant un tel intervalle est désormais partagée avec la source OSV au lieu d’être écrite une fois par source, ce qui est précisément la raison de leur désaccord initial.',
                'Les tests qui passaient à côté de tout cela génèrent désormais leurs entrées au lieu de les énumérer. Chaque plage de versions distincte que gemnasium publie pour npm est vérifiée contre l’implémentation de npm à chaque compilation, aux côtés du produit cartésien complet de la grammaire des plages et de tous les ordonnancements des événements de plage d’OSV. Exécuté sur la version précédente, ce balayage échoue sur 28 plages, alors que les dix exemples écrits à la main qu’il remplace passaient tous. Toute graphie inventée en amont à partir de maintenant fera échouer la compilation dès son premier import, et non le jour où quelqu’un remarquera le résultat qu’elle a produit.',
                'Chaque ligne du code qui produit ces résultats est désormais exercée par la suite de tests : instructions, branches, fonctions et lignes à 100 %, sans aucune exemption dans le dépôt. C’est la réponse directe à la façon dont les bogues des dernières versions sont passés : chacun était une branche que rien n’exécutait, signalant un succès sans rien faire. Fermer les 109 derniers en a révélé plusieurs classés comme garde-fous inatteignables alors qu’ils n’étaient tout simplement pas testés, ainsi qu’une règle de couverture qui ne vérifiait plus rien depuis que le fichier qu’elle nommait avait été déplacé. Cette règle et les 500 lignes d’exceptions qui l’entouraient sont remplacées par une seule, si bien qu’un code non testé fait maintenant échouer la compilation au lieu d’abaisser une moyenne.',
                'Les dernières lacunes défensives sont également fermées : une borne supérieure OSV illisible laisse l’intervalle valide ouvert, les bornes de secours sont vérifiées après leur application et une borne de préversion npm explicite comme <code>&lt;1.2.3-0</code> conserve son sens exact.',
                ...
                'Un cache qui finissait de se télécharger laissait chaque projet bloqué sur le verdict obtenu pendant son absence. Rien ne relançait d’analyse, donc <code>osv_db_not_seeded</code> restait là jusqu’au balayage planifié suivant, ou jusqu’à ce que vous le remarquiez et lanciez une analyse. Sentinello met désormais en file une analyse complète dès qu’un cache redevient utilisable. Une mise à jour incrémentale ne met rien en file.'
            ]
        },
        '3.3.2': {
    ...
    ja: {
        '3.4.0': {
            ...
            items: [
                'gemnasium の一部のアドバイザリは、比較演算子とバージョンの間に空白を入れて書かれています — <code>&lt;0.5.2</code> ではなく <code>&lt; 0.5.2</code> です。パーサーはこの組を 2 つの別々のトークンとして読んでいました。取り残された <code>&lt;</code> は空の境界を受け取り、単独で残ったバージョンは厳密な固定値としてキャッシュされます。その結果、<code>fresh</code> のアドバイザリはバージョン 0.5.2 を脆弱と報告していました。0.5.2 こそがこの問題を修正したリリースであるにもかかわらずです。しかも修正版なしとして報告されます。固定されたバージョンはアップグレード先をまったく持たないからです。この書き方のアドバイザリは 19 件あり、そのすべてが誤って解析されていました。15 件は範囲を完全に失い、7 件はレコード自身が修正版として挙げているバージョンを固定していました。<code>pg</code> では 11 件です。',
                'Sentinello は npm の範囲構文を自分で解釈するのをやめ、まず npm 自身の実装に範囲を渡すようになりました。これにより、一連の誤読がまとめて解消されます。npm にとって <code>&lt;=3.3</code> は「3.3 系列の最後まで」を意味します。文字どおりに読むと 3.3.0 で止まり、3.3.1 を見落としていました。これは実在する <code>converse.js</code> のアドバイザリです。また <code>=103</code> は単一の点ではなく 103 系列全体を意味し、<code>binaryen</code> は 8 件でこの書き方をしています。キャレット、チルダ、<code>1.x</code>、ハイフンの範囲も読めるようになりました。以前はいずれかで書かれたレコードは何も言わずに捨てられていました。gemnasium が npm 向けに公開する 4,696 件すべての個別バージョン範囲で確認したところ、自前で解釈すると npm と 9 件food食い違い、委譲すると 1 件も食い違いません。影響を受けるのは npm だけです。Python の <code>==1.0</code> はワイルドカードではなく厳密なバージョンなので、そこに npm の規則を当てはめると検出を捏造してしまいます。',
                'npm の読み方はパッケージをインストールするうえでは正しく、アドバイザリとしては誤っている点が 3 つあり、それらは適用を見送っています。バージョンをまったく挙げない範囲 — <code>*</code>、<code>x</code>、空欄 — は、npm がインストールするときは「任意のバージョン」を意味しますが、アドバイザリでは「誰も記入しなかった」ことと区別がつきません。そのため、公開された全リリースに対する検出に変えるのではなく拒否します。末尾に紛れ込んだ <code>||</code> が、その手前の範囲をすべてに広げることもなくなりました。さらに <code>&gt;0</code> は全バージョンを意味したままです。<code>pandora-doomsday</code> は悪意あるパッケージをこの書き方で宣言しており、npm の読み方では 0.x 系がすべて無害と判定されてしまうからです。別件として、宣言された修正版が自身の影響範囲の開始位置と同じかそれより下にあるアドバイザリでは、その値を境界として差し込まなくなりました。何にも一致しない区間、つまり黙って報告しなくなる検出を生んでいたためです。そして、そうした区間を捨てる規則はソースごとに書くのではなく OSV と共有するようになりました。個別に書いていたことこそが、両者の食い違いの発端でした。',
                'これらすべてを見逃し続けてきたテストは、入力を列挙するのではなく生成するようになりました。gemnasium が npm 向けに公開している個別のバージョン範囲すべてを、ビルドのたびに npm 自身の実装と突き合わせます。あわせて範囲文法の全組み合わせと、OSV の範囲イベントのあらゆる順序も検証します。前リリースに対して実行すると、この一括検証は 28 件の範囲で失敗します。これが置き換えた手書きの 10 例はすべて通っていました。今後、上流が新しい書き方を生み出しても、誰かがその結果生じた検出に気づいた時点ではなく、最初に取り込まれた時点でビルドが失敗します。',
                'これらの検出結果を支えるコードのすべてがテストで実行されるようになりました。ステートメント、分岐、関数、行のいずれも100%で、リポジトリ全体に除外はひとつもありません。これは直近数リリースの不具合がなぜ表に出たのかへの直接の答えです。いずれも何も実行されない分岐で、何もしないまま成功を報告していました。最後の109件を塞ぐ過程で、到達不能な安全確認として記録されていたものが実際には単に未テストだった例がいくつも見つかり、さらに、参照先のファイルが移動して以来何も検査していなかったカバレッジ規則も判明しました。その規則と周囲の500行の例外はひとつの規則に置き換わり、テストのないコードは平均値を下げるのではなくビルドを失敗させます。',
                '最後の防御上の欠落も解消しました。読み取れない OSV の上限は有効な区間を開いたままにし、フォールバック境界は適用後に検証され、<code>&lt;1.2.3-0</code> のような明示的な npm プレリリース境界は正確な意味を維持します。',
                ...
                'ダウンロードが完了したキャッシュがあっても、各プロジェクトはキャッシュが無かったときの判定のまま固定されていました。再スキャンが行われないため、<code>osv_db_not_seeded</code> は次の定期実行まで、あるいは気づいてスキャンを押すまで残り続けました。Sentinello はキャッシュが再び使用可能になった時点で、フルスキャンを 1 回キューに入れるようになりました。増分更新では何もキューに入れません。'
            ]
        },
        '3.3.2': {
    ...
    'zh-CN': {
        '3.4.0': {
            ...
            items: [
                ...
                '缓存下载完成后，每个项目仍停留在缓存缺失期间得到的结论上。没有任何机制重新扫描，因此 <code>osv_db_not_seeded</code> 会一直留在那里，直到下一次计划扫描，或者直到你注意到并手动点击扫描。现在只要缓存重新变得可用，Sentinello 就会立即排入一次全量重新扫描。增量更新则不会排入任何任务。'
            ]
        },
        '3.3.2': {
    ...
    ko: {
        '3.4.0': {
            ...
            items: [
                ...
                '캐시 다운로드가 끝나도 모든 프로젝트는 캐시가 없던 동안 받은 판정에 그대로 묶여 있었습니다. 다시 스캔하는 것이 없어 <code>osv_db_not_seeded</code>는 다음 예약 스캔까지, 또는 직접 알아차리고 스캔을 누를 때까지 남아 있었습니다. 이제 Sentinello는 캐시가 다시 사용 가능해지는 즉시 전체 재스캔을 한 번 대기열에 넣습니다. 증분 업데이트는 아무것도 넣지 않습니다.'
            ]
        },
        '3.3.2': {
...
export function getLatestRelease(): ReleaseEntry | null {
    // RELEASES is a non-empty tuple (see its declaration), so there is no empty case to fall back from.
    // The return type keeps `| null` for callers that already branch on it.
    return RELEASES[0]
}
