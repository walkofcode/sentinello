...
# Either way the fix is to delete the nvm cache volume and remount at /home/sentinello/.nvm. Note the
# volume's real name: docker compose prefixes named volumes with the project name, so it is usually
# `<project>_sentinello-nvm`, not a bare `sentinello-nvm` — both FATAL messages tell the operator to
# run `docker volume ls | grep sentinello` to find the exact name (deleting the wrong one silently
# leaves the offending volume in place and re-trips the guard).
ENTRYPOINT ["/usr/bin/tini", "--"]
CMD ["sh", "-c", "if grep -q ' /root/.nvm ' /proc/self/mountinfo 2>/dev/null; then echo '[sentinello] FATAL: the nvm cache volume is still mounted at the old /root/.nvm path from a pre-non-root release. It now lives at /home/sentinello/.nvm. Fix: change the mount to /home/sentinello/.nvm, then delete the old volume so it is recreated owned by the runtime user (it is a pure cache, nothing is lost). Volume name: docker compose prefixes it with your project (e.g. myproject_sentinello-nvm), so run `docker volume ls | grep sentinello` to find the exact name — deleting a bare sentinello-nvm may remove the wrong volume and re-trip this guard. See the README upgrade note. Refusing to start.' >&2; exit 1; fi; if [ ! -w $NVM_DIR ]; then echo '[sentinello] FATAL: /home/sentinello/.nvm is not writable by the non-root user — the nvm volume is root-owned from a pre-non-root release. Delete it so it is recreated fresh (it is a pure cache, nothing is lost). Volume name: docker compose prefixes it with your project (e.g. myproject_sentinello-nvm), so run `docker volume ls | grep sentinello` to find the exact name — deleting a bare sentinello-nvm may remove the wrong volume and re-trip this guard. See the README upgrade note. Refusing to start.' >&2; exit 1; fi; exec pm2-runtime start ecosystem.config.js"]
