...
//
// This is deliberately NOT the entry point. index.ts is, and it stays a three-line bin that imports
// main() from here and runs it — because ecosystem.config.js launches `src/index.ts` directly under
// `node --import tsx`, and the comment there records that an extra wrapper process once stopped
// SIGTERM from ever reaching the worker: the shutdown handler never ran, an in-flight scan died
// mid-write, and the single-instance lock was left held until it aged out as stale. Renaming or
// relocating the bin would put that back. Splitting the body out costs nothing at runtime (the bin
// still imports exactly this module) and makes main() and makeShutdown() reachable from a test
// process, which importing index.ts never could — importing it boots a worker.
...
export function ensureLockFileExists(lockPath: string): void {
...
export function assertDataDirWritable(dbPath: string): void {
...
export function dataDirNotWritableMessage(dir: string, detail: string): string {
